...
Full Google Calendar event CRUD for agents: JSON in, JSON out, and every mutation snapshotted so it can be restored. Output shape is described in `schemas/gcal-schema.json`.
...
Commands emit JSON by default. Pass `-format toon` **before** the subcommand (git-style), or set `KIRIGO_FORMAT=toon`, to get [TOON](https://github.com/toon-format/toon-go) — a token-compact encoding of the same structure, aimed at LLM/agent contexts:
...
gcal get <event-id>                                     # trimmed shape (gcal get --raw <event-id> for the full resource)
...
Flags come before the positional `<event-id>`/`<op-id>` (standard Go flag parsing); a flag placed after it is rejected rather than silently ignored.
